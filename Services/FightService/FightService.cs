...
namespace dotnet_rpg.Services.FightService
{
    public class FightService : IFightService
    {
        ...
        public async Task<ServiceResponse<FightResultDto>> Fight(FightRequestDto request)
        {
            ...
            try
            {
                var character = await _context.Characters.Include(c => c.Weapon).Include(c => c.Skills).Where(c => request.CharacterId.Contains(c.Id)).ToListAsync();
        ...
        {
            var characters = await _context.Characters.Where(c => c.Fight > 0).OrderByDescending(c => c.Victories).ThenBy(c => c.Defeats).ToListAsync();
